...
def make_epsilon_greedy_policy(Q, G, epsilon, observation, deadline):
    ...
    lower, upper = 0, G[int(current_node)][int(next_edge)]["wc"] # Set the upper and lower limits
    mu, sigma = G[int(current_node)][int(next_edge)]["tx"], variance # Mean = mean tx, variance
    time_traversed = stats.truncnorm.rvs((lower - mu) / sigma, (upper - mu) / sigma, mu)
...
print("Network with Normal distribution. deadline: ",deadline,", variance: ",variance,", epsiodes: ", num_episodes)
G = prsnt()
best_path = [G.nodes[0]['name']] #SWITCH TO NODES
